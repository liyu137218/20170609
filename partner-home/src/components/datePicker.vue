...
<template>
  <div class="cov-vue-date">
    <div class="datepickbox">
      <input type="text" class="cov-datepicker" readonly="readonly" :placeholder="option.placeholder" v-model="date.time" :required="required" @click="showCheck" @foucus="showCheck" :style="option.inputStyle ? option.inputStyle : {}" />
    </div>
    ...
    methods: {
      ...
      limitFromTo: function limitFromTo(limit, days) {
        ...
        if (limit.from || limit.to) {
          days.map((day) =>{
            if (_this2.getLimitCondition(limit, day) || day.action == 'next' || day.action == 'previous') {
              day.unavailable = true;
            }
          });
        }
      ...
      showCheck: function showCheck() {
        if (this.date.time === '') {
          ...
          if (this.option.type === 'day' || this.option.type === 'min' || this.option.type === 'month') {
      ...
      picked: function picked() {
        ...
        this.$store.commit('UPDATE_SHOWCYCLE', 'DIY')
      },
