...
const createconfig = () => {
  console.log('window.location.hostname.indexOf(\'test.\')=',window.location.hostname,window.location.hostname.indexOf('test.'))
  let _env = 'release';
  let host = { //线上
    env: 3,
    ...
    weituo: 'http://weituo.caidao.hexun.com/manager/login.html', // 新委托账户
  ...
  if(window.location.hostname.indexOf('test.')>=0) { // 测试环境
    _env = 'test';
    host = {
      env: 2,
      cmp: 'http://test.partner.px.hexun.com/',
